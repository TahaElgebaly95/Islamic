  ...
  Widget build(BuildContext context) {
    return BlocBuilder<QuranCubit, QuranStates>(
      builder: (context, state) {
        ...
        return cubit.searchController.text.isEmpty ? const SizedBox.shrink() : SizedBox(height: height * .18,
          child: ListView.separated(
            scrollDirection: Axis.horizontal,
            shrinkWrap: true,
            separatorBuilder: (context, index) => SizedBox(width: 10.w),
            itemBuilder: (context, index) =>
                MostRecentWidget(recentSurahs: cubit.surahs[index], ayahs: cubit.surahs[index].ayahs![index],),
